...
import { MessageSquare, Send, Phone, User, Search, Filter, MoreVertical, Reply, Image, FileText, Mic } from 'lucide-react';
import { useApp } from '../contexts/AppContext';
import { IncomingMessage, Conversation, OutgoingMessage, Contact } from '../types';
import { WhatsAppAPI } from '../utils/whatsappApi';
import { ProductionWebhookClient } from '../utils/webhookClient';

const InboxTab: React.FC = () => {
  const { contacts, setContacts, apiCredentials } = useApp();
  ...
  // Initialize production webhook client
  useEffect(() => {
    const webhookClient = ProductionWebhookClient.getInstance();
    
    // Set webhook URL if provided
    if (webhookUrl) {
      webhookClient.setWebhookUrl(webhookUrl);
    }
    ...
    webhookClient.onMessage((messages) => {
      console.log('📨 Processing messages from webhook:', messages);
  ...
  }, [webhookUrl, selectedConversation, conversations, setContacts]);
  ...
  const handleTestWebhook = async () => {
    ...
    if (result.success) {
      alert('✅ Webhook test successful! Check the inbox for a test message.');
    } else {
  ...
  const getCurrentWebhookUrl = () => {
    const webhookClient = ProductionWebhookClient.getInstance();
    return webhookClient.getWebhookUrl();
  };

  // Mock data for demonstration - In real app, this would come from webhook
  useEffect(() => {
  ...
  const handleSendReply = async () => {
    ...
    setSending(true);
    try {
      // In a real implementation, you'd send a free-form text message
      // For now, we'll simulate the reply
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4 text-sm text-gray-600">
          <div className="flex items-center">
            ...
            <span className={isWebhookConnected ? 'text-green-600' : 'text-red-600'}>
              Webhook {isWebhookConnected ? 'Connected' : 'Disconnected'}
            </span>
          </div>
          <div className="flex items-center">
      ...
      {/* Webhook Setup Notice - Moved to bottom with proper spacing */}
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mt-6">
        <div className="flex">
          ...
          <div className="text-sm text-blue-700">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium mb-2">Webhook Integration Status:</p>
                <p className={`text-sm ${isWebhookConnected ? 'text-green-700' : 'text-yellow-700'}`}>
                  {isWebhookConnected 
                    ? '✅ Ready to receive real WhatsApp messages' 
                    : '⚠️ Configure webhook in Settings to receive real messages'
                  }
                </p>
              </div>
            </div>
            <div className="mt-3 p-2 bg-yellow-100 border border-yellow-300 rounded">
              ...
              <p className="text-xs text-yellow-700 mt-1">
                When someone messages your WhatsApp Business number, the webhook will automatically add the conversation here and you can reply directly from this interface.
              </p>
            </div>
          </div>
        </div>
